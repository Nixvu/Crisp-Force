      ...
      <form action="../actions/proses_pelanggan.php" method="POST" id="addCustomerForm">
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          ...
          <div class="mt-4 space-y-4">
            ...
            <div><label class="block text-sm font-medium text-slate-700 mb-1">Segmentasi</label><select class="w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm" name="segmentasi" required><option value="baru">Baru</option><option value="loyal">Loyal</option><option value="perusahaan">Perusahaan</option></select></div>
          </div>
        </div>
      ...
      <form action="../actions/proses_pengguna.php" method="POST" id="editUserForm">
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          ...
          <div class="mt-4 space-y-4">
            ...
            <div id="edit_segmentasi_field" class="<?= $p['role'] == 'Customer' ? 'block' : 'hidden' ?>"><label class="block text-sm font-medium text-slate-700 mb-1">Segmentasi</label><select class="w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm" name="segmentasi" id="edit_segmentasi" required><option value="baru">Baru</option><option value="loyal">Loyal</option><option value="perusahaan">Perusahaan</option></select></div>
...
<script>
  ...
  function editUser(user) {
    ...
    const segmentasiField = document.getElementById('edit_segmentasi_field');
    const segmentasiSelect = document.getElementById('edit_segmentasi');

    if (user.role === 'Customer') {
      roleSelect.disabled = true;
      roleSelect.classList.add('bg-slate-200');
      segmentasiField.classList.remove('hidden');
      segmentasiSelect.value = user.segmentasi; // Set segmentasi value
    } else {
      roleSelect.disabled = false;
      roleSelect.classList.remove('bg-slate-200');
      segmentasiField.classList.add('hidden');
      segmentasiSelect.value = ''; // Clear segmentasi if not customer
    }
